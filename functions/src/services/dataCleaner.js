const { parseAndNormalizeTimestamp } = require('../utils/timestamp');
const { parseAndNormalizeLatency } = require('../utils/latency');

/**
 * Validates whether an HTTP status code is a valid standard HTTP response code.
 * (100 - 599). Specifically flags 999 or non-HTTP numbers as invalid.
 */
...
/**
 * Cleans, normalizes, deduplicates, and aggregates raw CSV log records.
 *
 ...
 */
function cleanAndNormalizeData(rawRecords, filename = 'monitoring_checks.csv') {
  const uploadSummary = {
    filename,
  ...
  for (let i = 0; i < rawRecords.length; i++) {
    ...
    if (!tsResult.isValid) {
      ...
      rejectedRecords.push({
        rowNumber,
        raw,
        reason: `Invalid timestamp: ${tsResult.error}`
      });
      continue;
    }
    ...
    if (!isValidHttpStatusCode(rawStatusCode)) {
      ...
      rejectedRecords.push({
        rowNumber,
        raw,
        reason: `Invalid HTTP status code: "${rawStatusCode}"`
      });
      continue;
    }
